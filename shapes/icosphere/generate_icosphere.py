...
def generate_icosphere_files():
	for i in range(1, 16):
		verts = icosphere(nu=i)[0]
		vertices_count = len(verts)
		with open(f'coordinates_{vertices_count}.txt', 'w') as file:
			file_content = '}\n{'.join(list(map(lambda x: ', '.join(list(map(str, x))), verts)))
...
def icosahedron():
	""""' Regular unit icosahedron. """  # 12 principal directions in 3D space: points on a unit icosahedron
	phi = (1 + np.sqrt(5)) / 2
	vertices = np.array([
...
def subdivide_mesh(vertices, faces, nu):
	...
	for e in range(E):
		edge = edges[e]
		for k in range(nu - 1):
			subvertices[V + e * (nu - 1) + k] = (w[-1 - k] * vertices[edge[0]] + w[k] * vertices[edge[1]])
